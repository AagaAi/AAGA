...
def run_tech_analysis(pair):
        ...
        for fv in fvgs:
            if fv["idx"] >= sw["idx"] and fv["idx"] <= sw["idx"] + 2:
                if sw["type"] == "bearish" and fv["type"] == "bullish":
                    ...
                    return signal_result("BUY", entry_zone, round(sl,2), round(tp,2), current)
                elif sw["type"] == "bullish" and fv["type"] == "bearish":
                    entry_zone = (fv["low"], fv["high"])
                    sl = sw["level"]
                    tp = fv["low"] - (fv["high"] - fv["low"])
                    return signal_result("SELL", entry_zone, round(sl,2), round(tp,2), current)

    # ---- Fallback: Simple Trend (ALWAYS sets entry_zone/SL/TP) ----
    ...
    if slope > 0:
        ...
        # Neutral market → still give a small range trade
        entry_zone = (current - 1.0, current + 1.0)
        sl = current - atr * 1.5
        tp = current + atr * 1.5
        return signal_result("BUY", entry_zone, round(sl,2), round(tp,2), current)
